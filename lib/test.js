    ...
    switch(result.code) {
      case 0:
        if(self.opt.type === 'repomake' && output.indexOf('make: Nothing to be done for') != -1) {
          callback({'result': 'nottested', 'output': self.output, 'code': result.code});
        } else {
          //mocha fix for empty tests
          if(output.indexOf(' 0 passing') != -1) {
            callback({'result': 'nok', 'output': self.output, 'code': result.code});
          } else {
            callback({'result': 'ok', 'output': self.output, 'code': result.code});
          }
        }
        break;
      case 100:
        self.origret = {'result': 'nottested', 'output': self.output, 'code': result.code};
      ...
      case 2:
        ...
        break;
      case 999:
        callback({'result': 'failed', 'output': self.output, 'code': result.code});
        break;
      case 137:
        if(self.opt.type === 'repomake') {
          callback({'result': 'nottested', 'output': self.output, 'code': result.code});
        } else {
          callback({'result': 'timedout', 'output': self.output, 'code': result.code});
        }
        break;
      case 8:
        callback({'result': 'failed', 'output': self.output, 'code': result.code});
      ...
      case 128:
        ...
        break;
      case 1:
        self.origret = {'result': 'nok', 'output': self.output, 'code': result.code};
...
Test.prototype.runRepoMake = function (callback) {
  ...
  self.globalOutput.stdout += '\n[OUTPUT] Testing using repo and make.\n';
...
Test.prototype.runRepo = function (callback) {
  ...
  self.globalOutput.stdout += '\n[OUTPUT] Testing using repo and npm.\n';
...
Test.prototype.runTarball = function (callback) {
  ...
  self.globalOutput.stdout += '\n[OUTPUT] Testing using tarball and npm.\n';
